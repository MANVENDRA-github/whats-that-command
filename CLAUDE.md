...
- **Scroll devices** (June 2026, all in `components/ui/`, all reduced-motion-safe):
  - `ScrollProgress` — phosphor line along the navbar's bottom border, scroll-linked `scaleX` through `useSpring` (transform-only); hidden entirely under reduced motion. Rendered inside `Navbar`, so it appears on every page.
  - `DecodeText` — terminal decode reveal: text resolves left-to-right from random ASCII glyphs when scrolled into view (`useInView`, once). Static export ships the final text, and mono fonts + constant char count mean zero layout shift. Used on landing kickers and value-prop titles. Reduced motion renders plain text.
  - Value-prop cards get a small scroll-linked parallax drift (`y` of 0/±18/±36px per column) — capped so stacked mobile cards (28px gap) can never collide.
  - `CRTReveal` — opacity-only power-on flicker (mirrors the `crt-on` body keyframes) for the inverted CTA block; reduced motion renders statically.
  - `Tilt3D` — mouse-tracking 3D perspective tilt (`rotateX`/`rotateY` via springs, `transformPerspective`) with a phosphor glare hotspot that follows the cursor (radial of `--glow-green-far`, clipped by an `overflow-hidden` layer). Marketing surfaces only: terminal demo frame (`max=5`), value-prop cards (`max=7`), CTA block (`max=2.5`, no glare). The demo also tilts upright on scroll (`rotateX` 10°→0). Mouse pointers only — `pointerType` guard ignores touch; reduced motion renders children untouched. No WebGL/Three.js — 3D is CSS transforms, keeping the bundle lean.
  - `WireframeBackdrop` (`components/landing/`) — a vector-CRT 3D wireframe **object** on a fixed background canvas (`-z-10`, pointer-events-none), home page only. Hand-rolled rotation + perspective projection on a 2D canvas — still no WebGL/Three.js. The models are hand-built vertex/edge lists of recognizable desk objects: **CRT monitor → floppy disk → coffee mug**, cycled by **clicking near the object** (background clicks only — interactive elements are excluded), with vertices lerping to the new shape and an amber flash mid-morph. Turntable spin + gentle sinusoidal nod (never tumbles upside down); **scroll** adds rotation and an upward parallax drift; **mouse** tilts it. Model space is y-up, canvas is y-down — projection negates y; new models must be authored y-up. Fades to 0 while a query is active (`active={!hasQuery}`), hidden below `sm`, dimmer below 1280px, and reduced motion gets a single static frame (click still swaps shapes instantly). **Stacking caveat:** the page background color + bloom live on `html` (NOT `body` — body must stay background-transparent) so the negative-z canvas paints between background and content; putting a background back on `body` will hide the canvas entirely.
